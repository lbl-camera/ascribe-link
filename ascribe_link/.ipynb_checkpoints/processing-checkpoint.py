...
def _type_to_schema(annotation: Any) -> dict[str, Any]:
    ...
    if origin in (Union, types.UnionType):
        ...
        union_schemas = [_type_to_schema(member_type) for member_type in args]
...
def _default_value_from_annotation(annotation: Any) -> Any:
    ...
    if origin is Annotated:
        return _default_value_from_annotation(args[0])

    if origin in (Union, types.UnionType):
        if len(args) == 2 and type(None) in args:
            non_none_type = args[0] if args[1] is type(None) else args[1]
            return _default_value_from_annotation(non_none_type)

        for member_type in args:
            if member_type is not type(None):
                value = _default_value_from_annotation(member_type)
...
def create_schema(func: Callable) -> dict[str, Any]:
    ...
    for param_name, param in sig.parameters.items():
        ...
        if param.default is inspect.Parameter.empty:
            resolved_default = _default_value_from_annotation(annotation)
            if resolved_default is not None:
                prop_schema["default"] = resolved_default
